...
enum AppBundleLaunch {
    ...
    static func findWisprAppBundle() -> URL? {
        ...
        for _ in 0..<10 {
            let candidate = dir.appendingPathComponent("Wispr.app", isDirectory: true)
        ...
        let homeApps = home.appendingPathComponent("Applications/Wispr.app", isDirectory: true)
        if FileManager.default.fileExists(atPath: homeApps.path) { return homeApps }
        let system = URL(fileURLWithPath: "/Applications/Wispr.app", isDirectory: true)
    ...
    static func relaunchThroughAppBundleIfNeeded() -> Bool {
        ...
        guard let appURL = findWisprAppBundle() else { return false }

        fputs("Relaunching via \(appURL.path) so Microphone/Accessibility apply to Wispr, not Terminal.\n", stdout)
        ...
        do {
            ...
            fputs("Error: could not start Wispr.app: \(error.localizedDescription)\n", stderr)
            return false
        }
